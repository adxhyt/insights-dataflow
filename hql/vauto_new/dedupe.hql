...
from vauto_recent_market_data where model_year >=2010 and vin is not null and created is not null  ) rm  where rm.group_rank=1;

drop table if exists  vauto_sold_market_vehicle_dedup;
CREATE TABLE vauto_sold_market_vehicle_dedup  AS SELECT 
*
from  
( SELECT 
*,
...
from vauto.vauto_sold_market_vehicle where cast(model_year as int) >=2010 and vin is not null and created is not null  ) rm  where rm.group_rank=1;
