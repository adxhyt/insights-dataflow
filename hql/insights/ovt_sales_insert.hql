...
coalesce(cc.division, ovt_make_model.make_desc, ext.ad_make_desc, vdmv.vb_make, mmr.mmr_make) as make,
coalesce(cc.model_name, ovt_make_model.make_desc, ext.ad_model_desc, vdmv.vb_make) as makeref,
 'n/a' as registration,
 'n/a' as chassis,
coalesce(cc.style_name, ovt_make_model.trim_desc, ext.ad_trim_desc, vdmv.ev_trim) as derivative,
abs(hash(coalesce(cc.style_name, ovt_make_model.trim_desc, ext.ad_trim_desc, vdmv.ev_trim))) as derivativeid,
...
coalesce(cc.model_name, ovt_make_model.model_desc, ext.ad_model_desc,vdmv.vb_model,  mmr.mmr_model) as model,
'n/a' as code,
coalesce(cc.model_year, ovt_reg.model_yr, ext.ad_year, cast(vdmv.vb_model_year as int), mmr.mmr_model_year) as modelyear,
coalesce(cc.model_name, ovt_make_model.model_desc, ext.ad_model_desc, vdmv.ev_model_id) as model_code,
...
auction.auction_nm as ovt_auction,
...
ext.ad_engine_desc as ovt_engine,
concat_ws(', ', 
...
from 
 ...
 join ovt.ovt_seller_customer_reg_dedup osc on osc.reg_key = ovt_reg.reg_key
 join ovt.man_ovt_dim_auction auction on auction.auction_key = ovt_reg.auction_key and ovt_reg.auction_key >=0
left join ovt.make_model_metrics_dedup om on om.reg_key = ovt_reg.reg_key
...
left join (select cust_key, first(bus_subtype_desc) as bus_subtype_desc, max(unix_timestamp(row_end_dt, 'yyyy-mm-dd hh:mm:ss')) from ovt.man_ovt_dim_customer group by cust_key ) as cust on ovt_reg.seller_cust_key=cust.cust_key;
