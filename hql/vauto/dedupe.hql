...
*,
 ...
 case 
   when unix_timestamp(created, 'MM/dd/yyyy HH:mm:ss aa') is null then unix_timestamp(created, 'dd-MMM-yy hh.mm.ss.SSSSSS aa' ) 
   else unix_timestamp(created, 'MM/dd/yyyy HH:mm:ss aa') end as created_ts, 
row_number() over ( partition by vin,
 case 
   when unix_timestamp(created, 'MM/dd/yyyy HH:mm:ss aa') is null then unix_timestamp(created, 'dd-MMM-yy hh.mm.ss.SSSSSS aa' ) 
   else unix_timestamp(created, 'MM/dd/yyyy HH:mm:ss aa') end, 
 is_certified order by
